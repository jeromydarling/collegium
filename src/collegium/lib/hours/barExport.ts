...
function renderJurisdictionText(
  ...
  switch (req.jurisdiction) {
    ...
    case "US-IL":
      return renderIllinois(req, s);
    case "US-MD":
      return renderMaryland(req, s);
    case "US-DC":
      return renderDistrictOfColumbia(req, s);
    case "US-FL":
    case "US-TX":
    default:
...
/* ------------------------------------------------------------------ */
/* Illinois — mandatory voluntary reporting (ARDC Annual Reg.)          */
/* ------------------------------------------------------------------ */

function renderIllinois(req: BarExportRequest, s: BarReportSummary): string {
  const lines: string[] = [];
  lines.push(`STATE OF ILLINOIS`);
  lines.push(`ATTORNEY REGISTRATION & DISCIPLINARY COMMISSION (ARDC)`);
  lines.push(`PRO BONO HOURS DISCLOSURE — ANNUAL REGISTRATION`);
  lines.push(`(Illinois Supreme Court Rule 756(f))`);
  lines.push(``);
  lines.push(`Attorney: ${s.attorneyName}`);
  if (s.barNumber) lines.push(`ARDC #: ${s.barNumber}`);
  if (req.attorney.firmOrClinic) lines.push(`Firm / Clinic: ${req.attorney.firmOrClinic}`);
  lines.push(`Reporting Year: ${s.reportingYear}`);
  lines.push(``);
  lines.push(
    `Pursuant to Illinois Supreme Court Rule 756(f), every Illinois attorney must disclose, in connection with annual registration, the approximate number of pro bono and qualifying public-service hours performed during the registration year.`
  );
  lines.push(``);

  lines.push(`I. PRO BONO LEGAL SERVICE`);
  lines.push(``);
  lines.push(`  Total qualifying pro bono hours: ${s.totalHours.toFixed(2)}`);
  lines.push(`  Aspirational target (R. 756(f) comment): 50 hours / year`);
  lines.push(``);
  lines.push(`II. SERVICE BY CATEGORY`);
  lines.push(``);
  for (const c of s.byCategory) {
  ...
  lines.push(``);
  lines.push(`III. MONETARY CONTRIBUTION TO LEGAL AID`);
  lines.push(``);
  lines.push(`  Amount contributed in lieu / in addition: $[AMOUNT, IF ANY]`);
  lines.push(``);
  lines.push(`IV. MATTERS — SUMMARY`);
  lines.push(``);
  s.matters.forEach((m, i) => {
    lines.push(`  ${(i + 1).toString().padStart(2)}. ${truncate(m.goal, 70)}`);
    lines.push(`      ${m.hours.toFixed(2)} hr   ${m.closed ? "Closed" : "Ongoing"}`);
  });
  lines.push(``);
  lines.push(`Submitted: ${s.signedDate}`);
  lines.push(``);
  lines.push(`_______________________________`);
  lines.push(`${s.attorneyName}, ARDC #${s.barNumber ?? "________"}`);
  lines.push(``);
  lines.push(`[GENERATED BY COLLEGIUM PATROCINIUM · ${todayShort()}]`);
  return lines.join("\n");
}
...
/* ------------------------------------------------------------------ */
/* Maryland — mandatory IOLTA-paired reporting                          */
/* ------------------------------------------------------------------ */

function renderMaryland(req: BarExportRequest, s: BarReportSummary): string {
  const lines: string[] = [];
  lines.push(`STATE OF MARYLAND`);
  lines.push(`COURT OF APPEALS — IOLTA / PRO BONO REPORTING`);
  lines.push(`(Md. Rule 16-903 · Pro Bono Legal Service Reporting)`);
  lines.push(``);
  lines.push(`Attorney: ${s.attorneyName}`);
  if (s.barNumber) lines.push(`Maryland Bar #: ${s.barNumber}`);
  if (req.attorney.firmOrClinic) lines.push(`Firm / Clinic: ${req.attorney.firmOrClinic}`);
  lines.push(`Reporting Year: ${s.reportingYear}`);
  lines.push(``);
  lines.push(
    `Pursuant to Md. Rule 16-903, every Maryland attorney must report annually the pro bono legal service performed during the calendar year.`
  );
  lines.push(``);
  lines.push(`I. STATUTORY ASPIRATIONAL THRESHOLD`);
  lines.push(``);
  lines.push(`  Aspirational target (Rule 6.1): 50 hours / year`);
  lines.push(`  Total hours reported: ${s.totalHours.toFixed(2)}`);
  lines.push(
    `  Status: ${s.meetsThreshold ? "MEETS aspirational target" : "Below aspirational target"}`
  );
  lines.push(``);

  lines.push(`II. SERVICE BREAKDOWN`);
  lines.push(``);
  for (const c of s.byCategory) {
    lines.push(
  ...
  lines.push(`III. MATTERS REPRESENTED`);
  lines.push(``);
  s.matters.forEach((m, i) => {
    lines.push(`  ${(i + 1).toString().padStart(2)}. [${m.hours.toFixed(2)} hr · ${m.closed ? "Closed" : "Open"}] ${truncate(m.goal, 65)}`);
  });
  lines.push(``);
  lines.push(`Submitted: ${s.signedDate}`);
  lines.push(``);
  lines.push(`_______________________________`);
  lines.push(`${s.attorneyName}, MD Bar #${s.barNumber ?? "________"}`);
  lines.push(``);
  lines.push(`[GENERATED BY COLLEGIUM PATROCINIUM · ${todayShort()}]`);
  return lines.join("\n");
}
...
/* ------------------------------------------------------------------ */
/* District of Columbia — Bar Counsel voluntary reporting               */
/* ------------------------------------------------------------------ */

function renderDistrictOfColumbia(
  req: BarExportRequest,
  s: BarReportSummary
): string {
  const lines: string[] = [];
  lines.push(`DISTRICT OF COLUMBIA BAR`);
  lines.push(`PRO BONO SERVICE REPORT`);
  lines.push(`(D.C. R. Prof. Conduct 6.1 · Aspirational 50-Hour Target)`);
  lines.push(``);
  lines.push(`Attorney: ${s.attorneyName}`);
  if (s.barNumber) lines.push(`D.C. Bar #: ${s.barNumber}`);
  if (req.attorney.firmOrClinic) lines.push(`Firm / Clinic: ${req.attorney.firmOrClinic}`);
  lines.push(`Reporting Year: ${s.reportingYear}`);
  lines.push(``);
  ...
  lines.push(``);
  lines.push(`  Total pro bono hours: ${s.totalHours.toFixed(2)}`);
  lines.push(`  Aspirational target: 50 hours / year`);
  lines.push(
    `  Status: ${s.meetsThreshold ? "MEETS aspirational target" : "Below aspirational target"}`
  );
  lines.push(``);
  ...
  lines.push(``);
  lines.push(
    `Hours above reflect services to persons of limited means or to qualifying public-interest organizations in furtherance of D.C. R. Prof. Conduct 6.1.`
  );
  lines.push(``);
  lines.push(`Submitted: ${s.signedDate}`);
  lines.push(``);
  lines.push(`_______________________________`);
  lines.push(`${s.attorneyName}, D.C. Bar #${s.barNumber ?? "________"}`);
  lines.push(``);
  lines.push(`[GENERATED BY COLLEGIUM PATROCINIUM · ${todayShort()}]`);
  return lines.join("\n");
}
