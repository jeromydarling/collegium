...
function EssayEditorDialog({ essay, onClose, onSave, onPublish, onSyncToSatellite, onRegenerateImage, isSaving, isPublishing, isSyncing, isRegenerating, showAiPanel, setShowAiPanel }: {
  ...
  const { data: federatedApps } = useFederatedApps();
  // Satellites are non-hub apps (everything except thecros)
  const satelliteApps = (federatedApps || []).filter(a => !a.is_hub);
                ...
                <Select value={targetApp} onValueChange={setTargetApp}>
                  ...
                  <SelectContent>
                    ...
                    {satelliteApps.map(app => (
                      <SelectItem key={app.source_app} value={app.source_app}>{app.display_name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {targetApp !== 'thecros' && (
        ...
        <DialogFooter className="flex-wrap gap-2">
          ...
          {canSyncToSatellite && (
            <Button size="sm" variant="secondary" onClick={() => onSyncToSatellite(targetApp)} disabled={isSyncing || isDirty} className="gap-1.5" title={isDirty ? 'Save first' : `Send to ${targetApp}`}>
              {isSyncing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />} Send to {satelliteApps.find(a => a.source_app === targetApp)?.display_name || targetApp}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
